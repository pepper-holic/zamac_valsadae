...
**남은 것**: 실제 사용자 계정으로 앱을 켜서 로그인 → 번역까지 수동 확인(이번 라운드는
자동화 테스트로만 검증), 구글 소셜 로그인, OpenAI 실제 키로 교체(`server/.env`의
`OPENAI_API_KEY`/`OPENAI_BASE_URL` 변경 + `OPENAI_MODEL_OVERRIDE` 비우기 — 코드 변경 불필요),
AI 검수 자동화(#44 후반부, 여전히 범위 밖).
