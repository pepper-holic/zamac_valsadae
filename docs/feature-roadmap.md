...
| 44 | 번역/AI 검수 서버 API 연동 | 오라클 VM 릴레이(`https://168-110-107-78.nip.io`)에서 Supabase 인증 → Gemini(무료) 번역 호출까지 end-to-end 성공 확인(2026-08-13) — 아래 상세 참고. AI 검수 자동화(수동 파일 왕복 → 서버 자동 호출)는 후속 라운드로 분리, 아직 미착수 | [~] 서버 end-to-end 완료, 앱 로그인 UI 대기 (2026-08-13) |
| 45 | 로그인/계정 시스템 실제 구현 | Supabase Auth 프로젝트 생성 + JWT 검증 실제 동작 확인 완료(2026-08-12). 데스크톱 앱의 실제 로그인 UI(회원가입/로그인 화면, 세션 토큰 저장 — Toolbar/App.tsx)가 없어서 사용자가 `TRANSLATION_API_KEY`에 넣을 JWT를 발급받을 방법이 아직 없음 — 다음 착수 대상 | [~] 서버 측 인증 완료, 앱 로그인 UI 미착수 (2026-08-13) |
...
**남은 것**: 로컬 데스크톱 앱에 로그인 화면이 없어서, `TRANSLATION_API_KEY`에 넣을 "사용자
JWT"를 발급받을 방법이 없음 — Supabase Auth 기반 로그인 UI(회원가입/로그인, 세션 토큰 저장)를
`frontend/`에 추가해야 실사용자가 이 서버를 쓸 수 있음. 나중에 OpenAI로 바꾸고 싶으면
`.env`의 `OPENAI_API_KEY`/`OPENAI_BASE_URL`을 바꾸고 `OPENAI_MODEL_OVERRIDE`를 비우면 됨(코드
변경 불필요).
