...
const AI_REVIEW_PROMPT = `# 자막 전사·번역 검수 프롬프트 (범용 템플릿)

아래 내용을 그대로 복사해서, 검수할 JSON 파일과 함께 매번 붙여넣어 사용하세요. 특정 분야(밴드/인터뷰 등)에 한정되지 않고, 어떤 주제의 영상이든 스스로 주제를 파악해 그에 맞게 조사·교정하도록 설계되어 있습니다.
...
### 0. 주제 파악 (가장 먼저 수행)
- 본격적인 교정·번역에 들어가기 전, 전체 세그먼트를 훑어 이 영상이 어떤 분야/주제(예: 시사·정치, 과학, 음악, 스포츠, 요리, 게임, 뷰티, 경제, 종교, 역사, 일상 브이로그 등)를 다루는지 파악합니다.
- 파악한 주제에 따라 등장할 가능성이 높은 고유명사 유형(인명, 지명, 기관·단체명, 제품명, 작품명, 전문 용어 등)을 예상하고, 그에 맞춰 조사 전략을 세웁니다.
- 주제가 명확하지 않거나 여러 주제가 섞여 있다면, 세그먼트별로 문맥을 보며 유연하게 판단합니다.

### 1. JSON 포맷 및 구조 유지 (절대 규칙)
- \`item_id\`, \`media_filename\`, \`instructions\` 및 각 segment의 \`id\`, \`start\`, \`end\` 값은 절대 수정·누락하지 않습니다.
- segment의 개수, 순서, 타임코드는 변경하지 않습니다.
- \`text\`, \`translation\` 필드만 아래 규칙에 따라 채우거나 교정합니다.
...
- 같은 대상이 여러 세그먼트에 걸쳐 다르게 오인식된 경우, 전체 파일에서 일관되게 동일한 정정 결과로 통일합니다.

### 3. 번역 방향 판별 및 처리
- 각 segment \`text\`의 언어를 (2번 교정 이후 최종 text 기준으로) 판별합니다.
...
- 세그먼트가 문장 중간에서 끊기는 경우가 많으므로, 앞뒤 세그먼트와 이어 읽었을 때 전체 흐름이 자연스럽게 연결되도록 번역합니다(단, 세그먼트를 합치거나 나누지는 않습니다).
...
### 5. 대용량 파일 작업 방식
- 세그먼트 수가 많을 경우(수백 개 이상), 다음 순서로 진행합니다:
  1. 전체 세그먼트를 훑어 주제를 파악하고, 등장하는 고유명사·전문용어 목록을 뽑아 웹 검색으로 확인합니다.
  2. 확인된 교정 목록을 사용자에게 먼저 간단히 공유합니다.
  3. 세그먼트를 순차적인 배치로 나누어 번역을 진행하고, 규모가 커서 시간이 걸릴 것으로 예상되면 미리 안내합니다.
  4. 작업 완료 후 전체 파일을 프로그래밍적으로 병합하고, id/start/end 무결성 및 translation 누락 여부를 검증합니다.

### 6. 파일 출력
- 검수 완료 후, 최종 수정된 전체 JSON을 원본과 동일한 스키마로 저장해 다운로드 가능한 파일로 제공합니다.
- 파악한 영상 주제, 원문을 교정한 세그먼트 목록(수정 전/후 비교), 확신도가 낮아 그대로 둔 항목 목록을 요약해서 함께 보고합니다.

---

### 사용 방법
1. 위 템플릿 전체를 복사합니다.
2. 검수할 자막 JSON 파일을 함께 첨부합니다.
