...
- 이메일/비밀번호 로그인(Supabase Auth) — 로그인하면 API 키 설정 없이 클라우드 릴레이
  (`server/`)를 통해 번역 사용 가능. 로그인은 선택 사항, 안 해도 기존 로컬/수동 API 키
  흐름 그대로 동작.
- 클라우드 릴레이 서버(`server/`) 신규 — 오라클 클라우드 VM에 배포, Supabase JWT 검증 후
  서버가 보관한 OpenAI/Gemini 키로 번역 요청을 대리 호출. 계정당 분당 요청 수·요청 바디
  크기 제한 포함.
- 서비스 웹사이트(`website/`) 랜딩 페이지 리디자인 — 로켓 발사 일러스트(순수 SVG),
  한글 브랜드명("자막발사대"), 다운로드/가격 정책 페이지를 카드형으로 재구성.
- 웹사이트·릴레이 서버 개별 배포 스크립트(`deploy/deploy-website.ps1`,
  `deploy/deploy-relay.ps1`) — 릴레이는 배포 전 테스트 실패 시 중단, 배포 후 헬스체크
  실패 시 자동 롤백.
...
- GPU는 감지되지만 cuBLAS/cuDNN DLL이 없어 전사 자체가 실패하던 문제 — 실행 시점 실패도 감지해
  CPU로 자동 재시도.
- 렌더링/번역 작업이 오래 걸리는 동안 같은 프로젝트의 다른 아이템을 수정하면 그 수정이
  조용히 사라지던 데이터 유실 버그 — `ProjectStore.update_item()`을 거치도록 수정
  (`export.py`, `translate.py`).
- `project.json` 저장이 원자적이지 않아 저장 도중 크래시 시 파일이 깨질 수 있던 문제 —
  temp 파일 + `os.replace`로 변경.
- ffmpeg가 진행률 출력 없이 멈추면 취소 요청이 반영되지 않던 문제 — 폴링 방식으로 변경,
  일정 시간 무응답 시 자동 중단.
